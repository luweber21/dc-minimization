...
for seed in range(5):
    env.reset()
    model = A2C("MultiInputPolicy", env, verbose=1, seed=seed)
    model.learn(total_timesteps=24000000, log_interval=np.inf, progress_bar=True)
    model.save(f"cleps/a2c_{seed}")
