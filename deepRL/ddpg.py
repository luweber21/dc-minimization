...
for seed in range(5):
    env.reset()
    model = DDPG("MultiInputPolicy", env, verbose=1, seed=seed)
    model.learn(total_timesteps=1900000, log_interval=np.inf, progress_bar=True)
    model.save(f"cleps/ddpg_{seed}")
